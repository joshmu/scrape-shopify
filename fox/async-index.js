...
(async () => {
  ...
  let remainingStores = urls.length;
  ...
  let results = await Promise.map(
    ...
    async (url, urlsIndex) => {
      ...
      remainingStores--;
      let remainingTime = await eta(
        completedCount,
        appStartTime,
        remainingStores
      );

      let txt = `\n-- ${urlsIndex}/${urls.length} -- ${remainingTime}${remainingTime <= 1 ? "min" : "mins"}\n`
      txt += url.toUpperCase() + '\n'
      txt += '---\n'
    //   console.log("");
    //   console.log(
    //     `-- ${urls.length - remainingStores}/${
    //       urls.length
    //     } -- ${remainingTime}${remainingTime <= 1 ? "min" : "mins"}`
    //   );
      ...
      if (urlCheck && updateCheck) {
        ...
        if (mostRecent.length) {
          ...
          try {
            ...
            console.error(e);
            bestSellers = [];
            txt += `setting bestsellers to empty array...${bestSellers}`
